...
// Convert 12-hour time format into total minutes for sorting
...
// Convert HTML time input (24-hour) into a readable 12-hour format
...
// Remove duplicate slots and sort them chronologically for each date
...
export default function EditProfilePage({ apiBase }) {
  ...
  useEffect(() => {
    let cancelled = false;
    // Load doctor details from the API when the page opens
  ...
  // Display temporary notification messages to the user
  ...
  // Add a new available date to the doctor's schedule
  ...
  // Add a new time slot to a specific schedule date
  ...
  // Remove a specific time slot from the selected date
  ...
  // Remove an entire date and all associated time slots
  ...
  // Handle local image selection and update preview before saving
  ...
  // Switch doctor availability between available and unavailable
  ...
  // Restore profile data from the server and discard local changes
  ...
  // Save profile updates and schedule changes to the backend
...
function AddDate({ onAdd }) {
  ...
  // Handle adding a new schedule date from the date picker
  const handleAdd = () => {
    if (value) {
      onAdd(value);
      setValue("");
