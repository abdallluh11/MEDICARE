...
const AppointmentPage = () => {
  ...
  const loadServiceAppointments = useCallback(async () => {
    ...
    try {
      ...
      console.log("Service appointments response:", resp?.data);

      const fetched = resp?.data?.appointments ?? resp?.data?.data ?? [];
      const arr = Array.isArray(fetched) ? fetched : [];
  ...
  const appointmentData = useMemo(() => {
    return doctorAppts
      .map((a) => {
        ...
        const status =
          a.status || (a.payment?.status === "Paid" ? "Confirmed" : "Pending");
  ...
  const serviceData = useMemo(() => {
    return serviceAppts
      .map((s) => {
        ...
        const status =
          s.status || (s.payment?.status === "Paid" ? "Confirmed" : "Pending");
        ...
        <div className={appointmentPageStyles.serviceGrid}>
          {serviceData.map((srv) => (
            <div key={srv.id} className={cardStyles.serviceCard}>
              ...
              <p className={cardStyles.price}>EGP{srv.price}</p>
