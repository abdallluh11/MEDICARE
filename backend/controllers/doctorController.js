...
// HElPER FUNCTION
// this function will convert time string like "2:30 PM" to total minutes from midnight, which is useful for sorting time slots
...
// this function accepts a schedule input which can be either a JSON string or an object, and returns a normalized schedule object with deduped and sorted time slots for each date
...
// this function will convert the raw doctor document from the database into a format that is more suitable for sending to the client, especially handling the schedule field which is stored as a Mongoose Map but needs to be sent as a plain object
...
export async function createDoctor(req, res) {
  try {
    ...
    const doc = new Doctor({
      ...
      about: body.about || "",
      fee:
        body.fee !== undefined && body.fee !== "" && !isNaN(Number(body.fee))
          ? Number(body.fee)
          : 0,
      schedule,
