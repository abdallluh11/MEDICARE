...
// Helpers functions
// this functions will return a finite number
...
// this functions will parse the time and return Am : Pm
...
export const confirmServiceAppointment = async (req, res) => {
  try {
    ...
    console.error("confirmServiceAppointment Error:", err);
    return res.status(500).json({
      success: false,
...
export const getServiceAppointmentStats = async (req, res) => {
  try {
    ...
    return res.json({ success: true, services, total: services.length });
  } catch (err) {
    console.error("getServiceAppointmentStats Error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
// to get appointments for the patient
export const getPatientAppointmentsByPatient = async (req, res) => {
  try {
    const clerkUserId = resolveClerkUserId(req);
    const { createdBy, mobile } = req.query;
    const resolvedUserId = createdBy || clerkUserId || null;
    ...
    console.error("getPatientAppointmentsByPatient Error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
