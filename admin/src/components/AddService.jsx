...
const AddService = ({ serviceId }) => {
  ...
  const [hasExistingImage, setHasExistingImage] = useState(false); // toUpdate the image if found
  ...
  // inital values as strings matching options
  ...
  // user cant select previous year/ month / date. from tody
  useEffect(() => {
  ...
  }, [serviceId, API_BASE]); //pre fetch that particular service if present
  ...
  // reset the form back to inital stat
  function resetForm() {
    if (imagePreview && imagePreview.startsWith("blob:")) {
      try {
        URL.revokeObjectURL(imagePreview);
      ...
      <div className={addServiceStyles.toast.container}>
        {toast && (
          ...
          >
            ...
            >
              <XCircle className=" w-4 text-gray-400 hover:text-gray-600" />
            </button>
          </div>
        )}
      </div>
